...
class VideoRenderer:
    ...
    def render(
        ...
        # Apply config font sizes so config.yaml controls the look
        if self.config.get("hook_font_size"):
            template.hook_font_size = self.config["hook_font_size"]
        if self.config.get("body_font_size"):
            template.body_font_size = self.config["body_font_size"]
        if self.config.get("caption_font_size"):
            template.caption_font_size = self.config["caption_font_size"]
