...
class Pipeline:
    ...
    def _build_groq_client(self):
        ...
        from src.utils.llm import RotatingKeyClient
        keys = []
        for var in ("GROQ_API_KEY_1", "GROQ_API_KEY_2", "GROQ_API_KEY_3", "GROQ_API_KEY_4", "GROQ_API_KEY_5", "GROQ_API_KEY_6", "GROQ_API_KEY"):
            k = os.getenv(var, "")
            if k and k not in keys:
                keys.append(k)
        if not keys:
    ...
    def discover(
        ...
        # CryptoPanic — always included when key is present; surfaces community-vetted
        # trending stories that NewsData and RSS may not surface quickly enough.
        if self.cryptopanic_client:
            try:
                stories = self.cryptopanic_client.search_stories(topic, max_results)
                all_stories.extend(stories)
